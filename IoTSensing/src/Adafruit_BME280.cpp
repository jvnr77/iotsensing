...
{
  if (_cs == -1) {
	_i2c->writeReg(reg,value);
  } else {
	  uint16_t txData=0;
	  txData = value << 8;
	  txData |= (reg & ~0x80);
	  _spi->write_word(txData);
  }
}
...
uint16_t Adafruit_BME280::read16(byte reg)
{
  ...
  if (_cs == -1) {
    ...
    value = _i2c->readWordReg(reg);

  } else {
	  uint8_t txData[3] = {};
	  uint8_t rxData[3]={};
	  txData[0] = reg;
	 _spi->transfer(txData,rxData,3);
	 value = (rxData[1]<<8) | rxData[0];
  }
